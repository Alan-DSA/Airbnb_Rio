...
sns.heatmap(colunas_numericas.corr(), annot=True, cmap='Greens')
plt.show()
...
# Defina as cores para os pontos de acordo com a categoria
plt.scatter(y_test, previsao, c=['blue' if p <= 200 else 'red' for p in previsao], alpha=0.5)
...
plt.scatter([], [], color='blue', label='Preço <= 200')
plt.scatter([], [], color='red', label='Preço > 200')
plt.legend()
